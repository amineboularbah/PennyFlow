...
class CalendarViewModel: ObservableObject {
    ...
    @Published var subscriptions: [Subscription] = [] // Holds fetched subscriptions

    private let context: NSManagedObjectContext // Core Data context for fetching data

    // MARK: - Initializer
    init(context: NSManagedObjectContext) {
        self.context = context
        self.selectedDate = Date() // Default to today's date
        fetchSubscriptions() // Load subscriptions from Core Data
    }
    ...
    /// Fetches subscriptions from Core Data.
